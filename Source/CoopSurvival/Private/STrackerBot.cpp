...
FAutoConsoleVariableRef CVARDebugTrackerBotDrawing(TEXT("COOP.DebugTrackerBot"), DebugTrackerBotDrawing, TEXT("Draw Debug Lines for Tracker Bot"), ECVF_Cheat);
...
void ASTrackerBot::BeginPlay()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		//Every second we update out powerlevel based on nearby bots
...
void ASTrackerBot::OnCheckNearbyBots()
{
	...
	//Our tracker bot's mesh component is se tto Physics Body in Blueprint (default profile of physics simulated actors)
	...
	{
		//Check if we overlapped with anothe tracker bot (ignoring players and other bot types)
	...
	if (MatInst)
	{
		...
		//this is a comming programming problem and can be fixed by 'casting' the int (MaxPowerLevel) to a float before dividing.
...
void ASTrackerBot::NotifyActorBeginOverlap(AActor* OtherActor)
{
	...
	if (!bStartedSelfDestruction && !bExploded)
	{
		...
		if (PlayerPawn && !USHealthComponent::IsFriendly(OtherActor,this))
		{
			...
			if (Role == ROLE_Authority)
			{
				//Start the self-destruction sequence.
